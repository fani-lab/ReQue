...
def compare_mAP_each_row(infile_df, outfile_path, col1, col2):
    ...
    for index, row in infile_df.iterrows():
        ...
        map1 = row[[word for word in map_columns if word.startswith(col1)][0]]
        map2 = row[[word for word in map_columns if word.startswith(col2)][0]]
        ...
        new_line.append(row[col1])  # qid
        new_line.append(map1)  # col 1 mAP
        new_line.append(row[col2])  # col 2 mAP
    ...
    #TODO Write another new excel just one time with extra col

    # write dataFrame to SalesRecords CSV file
    output_file_name = f'compare.mAP.{col1}.{col2}.csv'
...
def compare_mAP_all_row(infile_df, outfile_path, col1, col2):
    headers = list(infile_df.columns)
    map_columns = [word for word in headers if word.endswith("map")]
    map1_header = [word for word in map_columns if word.startswith(col1)][0]
    map2_header = [word for word in map_columns if word.startswith(col2)][0]

    infile_df.dropna(subset=[map1_header, map2_header])

    map1 = infile_df[[word for word in map_columns if word.startswith(col1)][0]].tolist()
    map1 = [0 if str(item) == 'nan' else item for item in map1]
    map2 = infile_df[[word for word in map_columns if word.startswith(col2)][0]].tolist()
    ...
    return mean1, mean2
    output_file_name = f'compare.mAP.{col1}.{col2}.txt'
    with open(outfile_path+output_file_name, 'w') as file:
        file.write(f'{col1} mean of mAP is: {mean1}\n')
        file.write(f'{col2} mean of mAP is: {mean2}\n')
...
if __name__ == '__main__':
    ...
    for index, each_col in enumerate(columns.col2):
        compare_mAP_each_row(infile_df=infile_df, outfile_path=outfile_path, col1=columns.col1, col2=each_col)
        compare_mAP_all_row(infile_df=infile_df, outfile_path=outfile_path, col1=columns.col1, col2=each_col)
